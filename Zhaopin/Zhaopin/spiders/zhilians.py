...
class ZhilianSpider(scrapy.Spider):
    ...
    def parse(self,response):
        ...
        yield scrapy.Request(url=start_url,callback=self.parse_cont,headers=self.headers,cookies=self.cookies)
    def parse_cont(self,response):
        ...
        base_url='http://sou.zhaopin.com/jobs/searchresult.ashx?jl=%s&ct=%s&el=%s&p=%s'
        for jl in dCity_list:
            print jl.encode('utf-8')
            for ct in dDegree_list:
                for el in dComptype_list:
                    for p in range(1,91):
                        full_url=base_url %(str(jl.encode('utf-8')),str(ct),str(el),str(p))
                        #print full_url
    ...
    def parse(self, response):
        ...
        # print html
        #print type(html)
        url_list = response.xpath('//td[@class="zwmc"]/div/a/@href').extract()
        # print url_list
        # print '-'*300
        if url_list:
            for url in url_list:
                print url
                yield scrapy.Request(url=url,callback=self.parse_details,headers=self.headers_x,cookies=self.cookies)
    def parse_details(self,response):
        ...
        company=f(response.xpath('//div[@class="tab-cont-box"]//h5/a[1]/text()').extract()[0])
         #工资
        money=response.xpath('//ul[@class="terminal-ul clearfix"]/li[1]/strong/text()').extract()
        moneya =''.join(money)
        moneys=re.compile(r'(\d+)')
        money_list=moneys.findall(moneya)
        if len(money_list)!=2:
            #最低工资
            salary_min= money_list[0]
            if salary_min is None:
                salary_min==''
            #最高工资
            salary_max= money_list[1]
        ...
        print position_type
         #标签
        tags=response.xpath('//div[@class="welfare-tab-box"]/span/text()').extract()
        if tags is None:
            tags ==''
        print ''.join(tags)
         #发布日期
        ...
        print pub_data
         #职位简介
        position_desc=response.xpath('//div[@class="tab-cont-box"]//p/text()').extract()
        if position_desc is None:
            position_desc=''
        #工作地址
        work_address=response.xpath('//div[@class="company-box"]//ul[@class="terminal-ul clearfix terminal-company mt20"]/li[4]/strong/text()').extract()
        ...
        print item['url'], item['company'],item['salary_min']
        yield item
    def md5(self,data):
        m = hashlib.md5()
        m.update(data)
